...
namespace nvh {
static uint32_t addPos(PrimitiveMesh& mesh, vec3 p)
{
  PrimitiveVertex v{};
  v.p = p;
...
static void addTriangle(PrimitiveMesh& mesh, vec3 a, vec3 b, vec3 c)
{
...
static void faceted(PrimitiveMesh& mesh)
{
  ...
  for(uint32_t i = 0; i < num_indices; i += 3)
  {
    ...
    auto n = (v1.p - v0.p).Unit().Cross((v2.p - v0.p).Unit());
...
PrimitiveMesh tetrahedron()
{
  ...
  // 4 vertices
  vec3 v0 = vec3{0.0F, 1.0F, 0.0F} * 0.5F;
  vec3 v1 = vec3{-c, -a, d} * 0.5F;
  vec3 v2 = vec3{-c, -a, -d} * 0.5F;
  vec3 v3 = vec3{b, -a, 0.0F} * 0.5F;
...
PrimitiveMesh plane(uint32_t steps, float width, float depth)
{
  ...
  for(uint32_t sz = 0; sz <= steps; sz++)
  {
    for(uint32_t sx = 0; sx <= steps; sx++)
    {
      ...
      v.p = vec3(-0.5F + (static_cast<float>(sx) * increment), 0.0F, -0.5F + (static_cast<float>(sz) * increment));
      v.p.Scale(vec3(width, 1.0F, depth));
      v.n = vec3(0.0F, 1.0F, 0.0F);
      v.t = vec2(static_cast<float>(sx) / static_cast<float>(steps),
...
PrimitiveMesh cube(float width /*= 1*/, float height /*= 1*/, float depth /*= 1*/)
{
  ...
  vec3              s   = vec3(width, height, depth) * 0.5F;
  std::vector<vec3> pnt = {{-s.X, -s.Y, -s.Z}, {-s.X, -s.Y, s.Z}, {-s.X, s.Y, -s.Z}, {-s.X, s.Y, s.Z},
                                    {s.X, -s.Y, -s.Z},  {s.X, -s.Y, s.Z},  {s.X, s.Y, -s.Z},  {s.X, s.Y, s.Z}};
  std::vector<vec3> nrm = {{-1.0F, 0.0F, 0.0F}, {0.0F, 0.0F, 1.0F},  {1.0F, 0.0F, 0.0F},
                                    {0.0F, 0.0F, -1.0F}, {0.0F, -1.0F, 0.0F}, {0.0F, 1.0F, 0.0F}};
  std::vector<vec2> uv  = {{0.0F, 0.0F}, {0.0F, 1.0F}, {1.0F, 1.0F}, {1.0F, 0.0F}};
