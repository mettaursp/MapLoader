                              ...
                              const uint32_t         numMips)
{
  ...
  mat4 mat_pers = mat4::NewProjection(90.0F, 1.0F, 0.1F, 10.0F);
  mat_pers               = mat_pers.Inverted();
  //mat_pers.a30 = mat_pers.a31 = mat_pers.a32 = mat_pers.a33 = 0.0F;

  std::array<mat4, 6> mv;
  const vec3          pos(0.0F, 0.0F, 0.0F);
  mv[0] = lookat(pos, vec3(1.0F, 0.0F, 0.0F), vec3(0.0F, -1.0F, 0.0F));   // Positive X
  mv[1] = lookat(pos, vec3(-1.0F, 0.0F, 0.0F), vec3(0.0F, -1.0F, 0.0F));  // Negative X
  mv[2] = lookat(pos, vec3(0.0F, -1.0F, 0.0F), vec3(0.0F, 0.0F, -1.0F));  // Positive Y
  mv[3] = lookat(pos, vec3(0.0F, 1.0F, 0.0F), vec3(0.0F, 0.0F, 1.0F));    // Negative Y
  mv[4] = lookat(pos, vec3(0.0F, 0.0F, 1.0F), vec3(0.0F, -1.0F, 0.0F));   // Positive Z
  mv[5] = lookat(pos, vec3(0.0F, 0.0F, -1.0F), vec3(0.0F, -1.0F, 0.0F));  // Negative Z
  for(auto& m : mv)
    m.Inverse();
  ...
  for(uint32_t mip = 0; mip < numMips; mip++)
  {
    for(uint32_t f = 0; f < 6; f++)
    {
      ...
      push_block.size       = {(float)extent.width, (float)extent.height};
