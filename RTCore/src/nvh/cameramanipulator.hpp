...
/**
  ...
  CameraManip.setLookat(eye, center, vec3(m_upVector == 0, m_upVector == 1, m_upVector == 2));
  CameraManip.getFov();
...
public:
  // clang-format off
    ...
    enum Actions { NoAction, Orbit, Dolly, Pan, LookAround, Pan2 };
    struct Inputs {bool lmb=false; bool mmb=false; bool rmb=false; 
    bool shift = false; bool ctrl = false; bool alt = false; bool alt2 = false;
    };
  // clang-format on
...
public:
  ...
  void setLookat(const vec3& eye, const vec3& center, const vec3& up, bool instantSet = true);
  ...
  vec3 getEye() const { return m_current.eye; }
  vec3 getCenter() const { return m_current.ctr; }
  vec3 getUp() const { return m_current.up; }
  ...
  void setMatrix(const mat4& mat_, bool instantSet = true, float centerDistance = 1.f);
  ...
  void keyMotion(float dx, float dy, float dz, int action);
  ...
  void                 setClipPlanes(vec2 clip) { m_clipPlanes = clip; }
  const vec2& getClipPlanes() const { return m_clipPlanes; }
  ...
  void fit(const vec3& boxMin, const vec3& boxMax, bool instantFit = true, bool tight = false, float aspect = 1.0f);
...
private:
  ...
  void update() {
      vec3 x, y, z;
      vec3 eye = m_current.eye;
      vec3 ctr = m_current.ctr;
      z = (eye - ctr).Unit();
      x = m_current.up.Cross(z).Unit();
      y = z.Cross(x).Unit();
      m_matrix = mat4(vec3(-x * eye, -y * eye, -z * eye), vec3(x.X, y.X, z.X), vec3(x.Y, y.Y, z.Y), vec3(x.Z, y.Z, z.Z));
  }
  ...
  vec3 computeBezier(float t, vec3& p0, vec3& p1, vec3& p2);
...
protected:
  ...
  vec2 m_mouse      = vec2(0.f, 0.f);
  vec2 m_clipPlanes = vec2(0.001f, 100000000.f);
