...
void TinyConverter::convert(tinygltf::Model& gltf, const tinyobj::ObjReader& reader)
{
  ...
  bufferEstimateSize += nbVertices * sizeof(vec3);
  bufferEstimateSize += normals.empty() ? 0 : nbVertices * sizeof(vec3);
  bufferEstimateSize += texcoords.empty() ? 0 : nbVertices * sizeof(vec2);
