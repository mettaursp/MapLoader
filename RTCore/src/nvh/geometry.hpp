...
struct Vertex
{
  Vertex(vec3 const& position, vec3 const& normal, vec2 const& texcoord)
      : position(vec4(position, 1.0f))
      , normal(vec4(normal, 0.0f))
      , texcoord(vec4(texcoord, 0.0f, 0.0f))
  {
  }
...
public:
  ...
  Plane(int segments = 1) { add(*this, mat4(1), segments, segments); }
};
...
{
public:
  static void add(Mesh<TVertex>& geo, const mat4& mat, int w, int h, int d)
  {
    int configs[6][2] = {
        {w, h}, {w, h},
    ...
    for(int side = 0; side < 6; side++)
    {
      ...
      mat4 matrixMove = nvmath::translation_mat4(0.0f, 0.0f, 1.0f);

      Plane<TVertex>::add(geo, mat * matrixRot * matrixMove, configs[side][0], configs[side][1]);
    }
  }

  Box(int segments = 1) { add(*this, mat4(1), segments, segments, segments); }
};
...
public:
  ...
  Sphere(int w = 16, int h = 8) { add(*this, mat4(1), w, h); }
};
...
public:
  static void add(Mesh<TVertex>& geo, const mat4& mat, int w, int h)
  {
    ...
    for(unsigned int latitude = 0; latitude <= (unsigned int)w; latitude++)  // theta angle
    {
      ...
      for(unsigned int longitude = 0; longitude <= (unsigned int)h; longitude++)  // phi angle
      {
        ...
        vec3 position = vec3(radius * cosPhi, outerRadius * sinTheta, radius * -sinPhi);
        vec3 normal   = vec3(cosPhi * cosTheta, sinTheta, -sinPhi * cosTheta);
        vec2 uv       = vec2((float)longitude / wf, (float)latitude / hf);
  ...
  Torus(int w = 16, int h = 16) { add(*this, mat4(1), w, h); }
};
...
{
public:
  static void add(Mesh<TVertex>& geo, const mat4& mat, int w, int h, int d, int level = 3, float probability = -1.f)
  {
    struct Cube
    {
      vec3 m_topLeftFront;
      ...
      void split(std::vector<Cube>& cubes)
      {
        ...
        vec3 topLeftFront = m_topLeftFront;
        for(int x = 0; x < 3; x++)
        {
    ...
    Cube cube = {vec3(-0.25, -0.25, -0.25), 0.5f};
    //Cube cube = { vec3(-25, -25, -25), 50.f };
    //Cube cube = { vec3(-40, -40, -40), 10.f };
    ...
    for(Cube& c : *previous)
    {
      mat4 matrixMove  = nvmath::translation_mat4(c.m_topLeftFront);
      mat4 matrixScale = nvmath::scale_mat4(vec3(c.m_size));
      ;
