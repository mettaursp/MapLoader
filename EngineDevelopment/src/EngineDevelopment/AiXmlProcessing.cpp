...
node.name="target" attribute sets:
		...
		<node name="target" type="rand"
			from to cooltime? initialCooltime? isKeepBattle?
		<node name="target" type="near"
			target? prob? from to cooltime? isKeepBattle?
			center cooltime? isKeepBattle?
		<node name="target" type="far"
			prob? from to cooltime? initialCooltime? isKeepBattle?
		<node name="target" type="mid"
			from to
		<node name="target" type="nearAssociated"
			from to
		<node name="target" type"rankAssociate"
			from to
			rank
		<node name="target" type="hasAdditional"
			additionalId additionalLevel target cooltime?
			additionalId additionalLevel target prob? isKeepBattle?
			additionalId additionalLevel target noChangeWhenNoTarget?
		<node name="target" type="randAssociated"
			(from to)?
		<node name="target" type="grabbedUser"
			from to

			(?!(((target="[^"]+" )?(prob="[^"]+" )?from="[^"]+" to="[^"]+" ?)|(center="[^"]+" ?))(cooltime="[^"]+" ?)?/?>)([^/]+/?>)
