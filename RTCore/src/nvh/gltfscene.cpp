...
void GltfScene::importMaterials(const tinygltf::Model& tmodel)
{
  ...
  for(auto& tmat : tmodel.materials)
  {
    ...
    gmat.emissiveFactor           = tmat.emissiveFactor.size() == 3 ?
                                        vec3((float)tmat.emissiveFactor[0], (float)tmat.emissiveFactor[1], (float)tmat.emissiveFactor[2]) :
                                        vec3(0.f);
    ...
    gmat.baseColorFactor =
        vec4((float)tpbr.baseColorFactor[0], (float)tpbr.baseColorFactor[1], (float)tpbr.baseColorFactor[2], (float)tpbr.baseColorFactor[3]);
    ...
    if(tpbr.baseColorTexture.extensions.find(KHR_TEXTURE_TRANSFORM_EXTENSION_NAME) != tpbr.baseColorTexture.extensions.end())
    {
      ...
      auto translation = mat3(vec3(0, 0, 1), vec3(1, 0, tt.offset.X), vec3(0, 1, tt.offset.Y));
      auto rotation = mat3(vec3(0, 0, 1), vec3(cos(tt.rotation), sin(tt.rotation), 0), vec3(-sin(tt.rotation), cos(tt.rotation), 0));
      auto scale     = mat3(vec3(0, 0, 1), vec3(tt.scale.X, 0, 0), vec3(0, tt.scale.Y, 0));
...
void GltfScene::importDrawableNodes(const tinygltf::Model& tmodel, GltfAttributes requestedAttributes, GltfAttributes forceRequested /*= GltfAttributes::All*/)
{
  ...
  for(auto nodeIdx : tscene.nodes)
  {
    processNode(tmodel, nodeIdx, mat4(1));
  }
...
//
//
void GltfScene::processNode(const tinygltf::Model& tmodel, int& nodeIdx, const mat4& parentMatrix)
{
  const auto& tnode = tmodel.nodes[nodeIdx];

  mat4 matrix      = getLocalMatrix(tnode);
  mat4 worldMatrix = parentMatrix * matrix;
                            ...
                            const std::string&         name)
{
  ...
  if(primMeshCached == false)  // Need to add this primitive
  {
    // POSITION
    {
      const bool hadPosition = getAttribute<vec3>(tmodel, tmesh, m_positions, "POSITION");
      if (!hadPosition) {
      ...
      {
        resultMesh.posMin = vec3((float)accessor.minValues[0], (float)accessor.minValues[1], (float)accessor.minValues[2]);
      }
      else
      {
      ...
      {
        resultMesh.posMax = vec3((float)accessor.maxValues[0], (float)accessor.maxValues[1], (float)accessor.maxValues[2]);
      }
      else
      {
    ...
    {
      bool normalCreated = getAttribute<vec3>(tmodel, tmesh, m_normals, "NORMAL");
    ...
    {
      bool texcoordCreated = getAttribute<vec2>(tmodel, tmesh, m_texcoords0, "TEXCOORD_0");
      if(!texcoordCreated)
        texcoordCreated = getAttribute<vec2>(tmodel, tmesh, m_texcoords0, "TEXCOORD");
    ...
    {
      bool tangentCreated = getAttribute<vec4>(tmodel, tmesh, m_tangents, "TANGENT");
    ...
    {
      bool colorCreated = getAttribute<vec4>(tmodel, tmesh, m_colors0, "COLOR_0");
...
void GltfScene::createNormals(GltfPrimMesh& resultMesh)
{
  ...
  for(size_t i = 0; i < resultMesh.indexCount; i += 3)
  {
    ...
    const auto  v1   = (pos1 - pos0).Unit();  // Many normalize, but when objects are really small the
    const auto  v2   = (pos2 - pos0).Unit();  // cross will go below nv_eps and the normal will be (0,0,0)
...
void GltfScene::createTexcoords(GltfPrimMesh& resultMesh)
{
  ...
  //      m_texcoords0.insert(m_texcoords0.end(), resultMesh.vertexCount, vec2(0, 0));
...
void GltfScene::createTangents(GltfPrimMesh& resultMesh)
{
  ...
  for(uint32_t a = 0; a < resultMesh.vertexCount; a++)
  {
    ...
    vec3 otangent = (t - ((n * t) * n)).Unit();

    // In case the tangent is invalid
    if(otangent == vec3(0, 0, 0))
    {
      if(abs(n.X) > abs(n.Y))
        otangent = vec3(n.Z, 0, -n.X) * (1 / sqrtf(n.X * n.X + n.Z * n.Z));
      else
        otangent = vec3(0, -n.Z, n.Y) * (1 / sqrtf(n.Y * n.Y + n.Z * n.Z));
    }

    // Calculate handedness
    float handedness = ((n.Cross(t) * b) < 0.0F) ? 1.0F : -1.0F;
...
{
  // Set them all to one
  m_colors0.insert(m_colors0.end(), resultMesh.vertexCount, vec4(1, 1, 1, 1));
}

//--------------------------------------------------------------------------------------------------
// Return the matrix of the node
//
mat4 getLocalMatrix(const tinygltf::Node& tnode)
{
  mat4 mtranslation{1};
  mat4 mscale{1};
  mat4 mrot{1};
  ...
  if(!tnode.translation.empty())
    mtranslation = mat4(vec3((float)tnode.translation[0], (float)tnode.translation[1], (float)tnode.translation[2]));
  if(!tnode.scale.empty())
    mscale = mat4::NewScale(vec3((float)tnode.scale[0], (float)tnode.scale[1], (float)tnode.scale[2]));
  if(!tnode.rotation.empty())
  {
    ...
    mrot = mrotation.MatrixF();
  }
  if(!tnode.matrix.empty())
  {
    for(int i = 0; i < 16; ++i)
      matrix.Fetch(i / 4, i % 4) = static_cast<float>(tnode.matrix[i]);
  }
...
void GltfScene::computeCamera()
{
  for(auto& camera : m_cameras)
  {
    ...
    {
        camera.eye = camera.worldMatrix.Translation();
      float distance = (m_dimensions.center - camera.eye).Length();
      mat3  rotMat   = camera.worldMatrix;
...
void GltfScene::exportDrawableNodes(tinygltf::Model& tmodel, GltfAttributes requestedAttributes)
{
  ...
  for(auto& n : m_nodes)
  {
    ...
    // Matrix -- Adding it only if not identity
    {
      ...
      for(int i = 0; i < 16; i++)
      {
        isIdentity &= (identityMat.Fetch(i / 4, i % 4) == n.worldMatrix.Fetch(i / 4, i % 4));
        matrix[i] = n.worldMatrix.Fetch(i / 4, i % 4);
      }
      if(!isIdentity)
        node->matrix = matrix;
    }
  ...
  if(!m_cameras.empty())
  {
    ...
    for(int i = 0; i < 16; i++)
      node.matrix[i] = m_cameras[0].worldMatrix.Fetch(i / 4, i % 4);

    // Add extension with pos, interest and up
    auto fct = [](const std::string& name, vec3 val) {
  ...
  for(const auto& pm : m_primMeshes)
  {
...

    {
      ...
      auto& tBufferView      = tmodel.bufferViews.back();
      tBufferView.buffer     = 0;
      tBufferView.byteOffset = olPos.offset + pm.vertexOffset * sizeof(vec3);
    ...
    if(hasFlag(requestedAttributes, GltfAttributes::Normal) && !m_normals.empty())
    {
      ...
      auto& tBufferView      = tmodel.bufferViews.back();
      tBufferView.buffer     = 0;
      tBufferView.byteOffset = olNrm.offset + pm.vertexOffset * sizeof(vec3);
      tBufferView.byteStride = 3 * sizeof(float);
      tBufferView.byteLength = pm.vertexCount * tBufferView.byteStride;
    ...
    if(hasFlag(requestedAttributes, GltfAttributes::Texcoord_0) && !m_texcoords0.empty())
    {
      ...
      auto& tBufferView      = tmodel.bufferViews.back();
      tBufferView.buffer     = 0;
      tBufferView.byteOffset = olTex.offset + pm.vertexOffset * sizeof(vec2);
    ...
    if(hasFlag(requestedAttributes, GltfAttributes::Tangent) && !m_tangents.empty())
    {
      ...
      auto& tBufferView      = tmodel.bufferViews.back();
      tBufferView.buffer     = 0;
      tBufferView.byteOffset = olTan.offset + pm.vertexOffset * sizeof(vec4);
    ...
    if(hasFlag(requestedAttributes, GltfAttributes::Color_0) && !m_colors0.empty())
    {
      ...
      auto& tBufferView      = tmodel.bufferViews.back();
      tBufferView.buffer     = 0;
      tBufferView.byteOffset = olCol.offset + pm.vertexOffset * sizeof(vec4);
      tBufferView.byteStride = 4 * sizeof(float);
      tBufferView.byteLength = pm.vertexCount * tBufferView.byteStride;
