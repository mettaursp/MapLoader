...
//
//
void AxisVK::display(VkCommandBuffer cmdBuf, const mat4& transform, const VkExtent2D& screenSize)
{
  ...
  // Set the orthographic matrix in the bottom left corner
  {
    ...
    const mat4 matOrtho = mat4(vec3(-1.f + pixelW, 1.f - pixelH,  0.5f),
                               vec3(pixelW * .8f,  0.0f,          0.0f),
                               vec3(0.0f,          -pixelH * .8f, 0.0f),
                               vec3(0.0f,          0.0f,          -0.5f));

    mat4 modelView = transform;
    modelView.SetTranslation({0, 0, 0});
    modelView = matOrtho * modelView;
    // Push the matrix to the shader
    vkCmdPushConstants(cmdBuf, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(mat4), &modelView.Fetch(0, 0));
  }
...
void AxisVK::createAxisObject(CreateAxisInfo& info)
{
  ...
  const VkPushConstantRange push_constants{VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(mat4)};
