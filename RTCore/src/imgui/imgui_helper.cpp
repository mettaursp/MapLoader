...
template <>
bool Control::show_slider_control<vec2>(vec2* value, vec2& min, vec2& max, const char* format)
{
  return show_slider_control_scalar<float, ImGuiDataType_Float, 2>(&value->X, &min.X, &max.X, format ? format : "%.3f");
}

template <>
bool Control::show_slider_control<vec3>(vec3* value, vec3& min, vec3& max, const char* format)
{
  return show_slider_control_scalar<float, ImGuiDataType_Float, 3>(&value->X, &min.X, &max.X, format ? format : "%.3f");
}

template <>
bool Control::show_slider_control<vec4>(vec4* value, vec4& min, vec4& max, const char* format)
{
...
template <>
bool Control::show_drag_control<vec2>(vec2* value, float speed, vec2& min, vec2& max, const char* format)
{
  return show_drag_control_scalar<float, ImGuiDataType_Float, 2>(&value->X, speed, &min.X, &max.X, format ? format : "%.3f");
}

template <>
bool Control::show_drag_control<vec3>(vec3* value, float speed, vec3& min, vec3& max, const char* format)
{
  return show_drag_control_scalar<float, ImGuiDataType_Float, 3>(&value->X, speed, &min.X, &max.X, format ? format : "%.3f");
}

template <>
bool Control::show_drag_control<vec4>(vec4* value, float speed, vec4& min, vec4& max, const char* format)
{
...
template <>
bool Control::show_slider_control<vec2i>(vec2i* value, vec2i& min, vec2i& max, const char* format)
{
  return show_slider_control_scalar<int, ImGuiDataType_S32, 2>(&value->X, &min.X, &max.X, format ? format : "%d");
}

template <>
bool Control::show_slider_control<vec3i>(vec3i* value, vec3i& min, vec3i& max, const char* format)
{
  return show_slider_control_scalar<int, ImGuiDataType_S32, 3>(&value->X, &min.X, &max.X, format ? format : "%d");
}

template <>
bool Control::show_slider_control<vec4i>(vec4i* value, vec4i& min, vec4i& max, const char* format)
{
...
template <>
bool Control::show_drag_control<vec2i>(vec2i* value, float speed, vec2i& min, vec2i& max, const char* format)
{
  return show_drag_control_scalar<int, ImGuiDataType_S32, 2>(&value->X, speed, &min.X, &max.X, format ? format : "%d");
}

template <>
bool Control::show_drag_control<vec3i>(vec3i* value, float speed, vec3i& min, vec3i& max, const char* format)
{
  return show_drag_control_scalar<int, ImGuiDataType_S32, 3>(&value->X, speed, &min.X, &max.X, format ? format : "%d");
}

template <>
bool Control::show_drag_control<vec4i>(vec4i* value, float speed, vec4i& min, vec4i& max, const char* format)
{
...
void Panel::Begin(Side side /*= Side::Right*/, float alpha /*= 0.5f*/, char* name /*= nullptr*/)
{
  ...
  if(!ImGui::DockBuilderGetNode(dockspaceID))
  {
    ...
    const float xRatio = clamp<float>(320.0f * getDPIScale() / viewport->WorkSize[0], 0.01f, 0.499f);
    const float yRatio = clamp<float>(180.0f * getDPIScale() / viewport->WorkSize[1], 0.01f, 0.499f);
...
bool ImGuiH::azimuthElevationSliders(vec3& direction, bool negative, bool yIsUp /*=true*/)
{
  vec3 normalized_dir = direction.Unit();
  if(negative)
  {
