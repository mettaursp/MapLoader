...
void nvvkhl::AppBaseVk::updateCamera()
{
  ...
  float factor = ImGui::GetIO().DeltaTime * 0.025f;// *10 * m_sceneRadius;
  ...
  m_inputs.lmb   = ImGui::IsMouseDown(ImGuiMouseButton_Right);
  ...
  m_inputs.alt2   = ImGui::IsKeyDown(ImGuiKey_LeftAlt) || ImGui::IsKeyDown(ImGuiKey_RightAlt);

  glfwGetCursorPos(m_window, &mousePosXF, &mousePosYF);

  glfwSetInputMode(m_window, GLFW_CURSOR, m_inputs.lmb ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL);
  ...
  CameraManip.keyMotion(0, 0, 0, nvh::CameraManipulator::NoAction);
  ...
  //if(!(ImGui::IsKeyDown(ImGuiKey_ModAlt) || ImGui::IsKeyDown(ImGuiKey_ModCtrl) || ImGui::IsKeyDown(ImGuiKey_ModShift)))
  {
    ...
    if(ImGui::IsKeyDown(ImGuiKey_UpArrow))
      CameraManip.keyMotion(0, factor, 0, nvh::CameraManipulator::Pan);

    if(ImGui::IsKeyDown(ImGuiKey_DownArrow))
      CameraManip.keyMotion(0, -factor, 0, nvh::CameraManipulator::Pan);

    if (dx != 0 || dy != 0 || dz != 0)
      CameraManip.keyMotion(dx, dy ,dz, m_inputs.alt2 ? nvh::CameraManipulator::Pan2 : nvh::CameraManipulator::Pan);
  }
...
//
void nvvkhl::AppBaseVk::initGUI(VkRenderPass renderPass, uint32_t subpassID /*= 0*/)
{
...
//
void nvvkhl::AppBaseVk::fitCamera(const vec3& boxMin, const vec3& boxMax, bool instantFit /*= true*/)
{
