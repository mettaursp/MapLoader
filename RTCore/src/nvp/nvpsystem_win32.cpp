...
static std::string fileDialog(struct GLFWwindow* glfwin, const char* title, const char* exts, bool openToLoad)
{
  ...
  auto tostring = [](const std::wstring& in)
  {
      std::string out;
      std::transform(in.begin(), in.end(), out.begin(), [](wchar_t c) { return (char)c; });

      return out;
  };
  auto towstring = [](const std::string& in)
  {
      std::wstring out;
      std::transform(in.begin(), in.end(), out.begin(), [](wchar_t c) { return (char)c; });
  ...
  auto lpwstr = [](std::wstring& in)
  {
      return reinterpret_cast<wchar_t*>(&in[0]);
  };
  std::wstring extsfixedW = towstring(extsfixed.data());
  std::wstring titleW = towstring(title);
  ...
  ofn.lpstrFilter     = lpwstr(extsfixedW);
  ofn.nFilterIndex    = 1;
  ofn.lpstrFileTitle  = NULL;
  ofn.nMaxFileTitle   = 0;
  ofn.lpstrInitialDir = NULL;
  ofn.Flags           = OFN_PATHMUSTEXIST;
  ofn.lpstrTitle      = lpwstr(titleW);
