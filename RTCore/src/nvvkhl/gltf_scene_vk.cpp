...
void nvvkhl::SceneVk::createInstanceInfoBuffer(VkCommandBuffer cmd, const nvh::GltfScene& scn)
{
  ...
  for(const auto& node : scn.m_nodes)
  {
    ...
    info.objMatrixIT = node.worldMatrix.Inverted().Transposed();
  ...
  for(const auto& prim_mesh : scn.m_primMeshes)
  {
    ...
    if(it == cache_primitive.end())
    {
      ...
      for(uint32_t v_ctx = 0; v_ctx < prim_mesh.vertexCount; v_ctx++)
      {
        ...
        const vec3& p   = scn.m_positions[idx];
        const vec3& n   = scn.m_normals[idx];
        const vec4& t   = scn.m_tangents[idx];
        const vec2& u   = scn.m_texcoords0[idx];

        Vertex& v  = vertices[v_ctx];
        v.position = vec4(p, u.X);  // Adding texcoord to the end of position and normal vector
        v.normal   = vec4(n, u.Y);
        v.tangent  = t;
      }
