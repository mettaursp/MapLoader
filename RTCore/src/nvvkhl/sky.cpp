...
static ProceduralSkyShaderParameters fillShaderParameters(const SkyParameters& input)
{
  ...
  float light_angular_size = clamp(input.angularSize, nv_to_rad * 0.1F, nv_to_rad * 90.F);
  ...
  output.directionToLight   = normalize(-input.direction);
  output.angularSizeOfLight = light_angular_size;
  output.lightColor         = light_radiance * input.color;
  output.glowSize           = nv_to_rad * clamp(input.glowSize, 0.F, 90.F);
  output.skyColor           = input.skyColor * input.brightness;
  output.glowIntensity      = clamp(input.glowIntensity, 0.F, 1.F);
  output.horizonColor       = input.horizonColor * input.brightness;
  output.horizonSize        = nv_to_rad * clamp(input.horizonSize, 0.F, 90.F);
  output.groundColor        = input.groundColor * input.brightness;
  output.glowSharpness      = clamp(input.glowSharpness, 1.F, 10.F);
...
void SkyDome::draw(const VkCommandBuffer& cmd, const mat4& view, const mat4& proj, const VkExtent2D& size)
{
  LABEL_SCOPE_VK(cmd);
...
bool SkyDome::onUI()
{
  ...
  vec3 dir = m_skyParams.direction;
