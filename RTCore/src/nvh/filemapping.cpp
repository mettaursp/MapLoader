...
bool FileMapping::open(const char* fileName, MappingType mappingType, size_t fileSize)
{
  ...
  if(mappingType == MAPPING_READOVERWRITE)
  {
    ...
    DWORD length = GetVolumePathNameA(fileName, dir, MAX_PATH + 1);

    if(length > 0)
    {
      success = GetDiskFreeSpaceExA(dir, NULL, NULL, &numFreeBytes);
    }
  ...
  m_win32.file = mappingType == MAPPING_READONLY ?
                     CreateFileA(fileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, NULL) :
                     CreateFileA(fileName, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
