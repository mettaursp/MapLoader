...
void StagingMemoryManager::cmdToImageDDS(VkDeviceSize bufferOffset, VkOffset3D& offset,
  VkExtent3D& extent,
  const VkImageSubresourceLayers& subresource,
  VkBufferImageCopy& mipLevel)
{
  VkBufferImageCopy& cpy = mipLevel;
  cpy.bufferOffset      = bufferOffset;
  cpy.bufferRowLength   = 0;
  cpy.bufferImageHeight = 0;
  cpy.imageSubresource  = subresource;
  cpy.imageOffset       = offset;
...
void* StagingMemoryManager::cmdToImageDDS(VkCommandBuffer                 cmd,
  VkImage                         image,
  VkDeviceSize                    size,
  const void* data,
  VkImageLayout                   layout,
  std::vector<VkBufferImageCopy>& mipLevels)
{
  ...
  for (VkBufferImageCopy& cpy : mipLevels)
    cpy.bufferOffset += srcOffset;

  vkCmdCopyBufferToImage(cmd, srcBuffer, image, layout, (uint32_t)mipLevels.size(), mipLevels.data());
